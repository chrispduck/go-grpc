...
import (
	"github.com/chrispduck/go-grpc/models/"
)
...
func NewService( /* a database connection would be injected here */ ) models.Service {
	return &service{
		m: map[int]models.User{
			1: {ID: 1, Name: "Alice"},
			2: {ID: 2, Name: "Bob"},
...
func (s *service) GetUser(id int) (result models.User, err error) {
...
func (s *service) GetUsers(ids []int) (result map[int]models.User, err error) {
	// always a good idea to return non-nil maps to avoid nil pointer dereferences
	result = map[int]models.User{}
	for _, id := range ids {
		if u, ok := s.m[id]; ok {
			result[id] = u
		}
